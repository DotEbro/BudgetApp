...
class _DashboardScreenState extends State<DashboardScreen> {
  ...
  double getThisMonAmount(int mon){
    ...
    double getCurrentAmount(){
      ...
      for(int i = 0; i < widget.allTransaction.length; i++){
        ...
        for(int j = 0; j < widget.allTransaction[i].credited.length; j++){
          Iterable<Match> matches = regEXP.allMatches(widget.allTransaction[i].credited, j);
          for (final Match m in matches) {
            credited = double.parse(m[0]!);
          }
          break;
        }
  ...
  initState(){
    ...
    startListening();
    setDataMap();
    double amount = getThisMonAmount(selected);
    currentMonAmount = double. parse((amount). toStringAsFixed(2));
    Current_amount = getCurrentAmount();
    double Amount_current = double. parse((Current_amount). toStringAsFixed(3));
  }
                ...
                Padding(
                  ...
                  child: Column(
                    ...
                    children: [
                      ...
                      Text(
                        "expenditure".tr +' ${monthList[nowmonth-1]}' + ' ${DateTime.now().year}',
                        style: TextStyle(color: Colors.white, fontSize: 18),
                      ),
                      const SizedBox(
                        height: 8,
                      ...
                      : Column(
                        children: [
                          ListView.builder(
                              ...
                              itemCount: showLess ? (widget.allTransaction.length >= 20 ? 20 : widget.allTransaction.length) : widget.allTransaction.length,
                          ...
                          Padding(
                            ...
                            child: Align(
                              ...
                              child: GestureDetector(
                                ...
                                child: Text(
                                  showLess ? "showMore".tr : "showLess".tr,
  ...
  Widget showTransaction(UserTransactionData data, BuildContext context){
    //var placeCat = "";
    //   if (data.place.contains("Talabat")){
    //     data.category = "r_o";
    //   }
    //   if (data.place.contains("Deliveroo")){
    //     data.category = "r_o";
    //   }
